...
def train(is_old_training: bool, train_path, test_path, checkpoint_dir, epochs):
    ...
    for epoch in range(epochs):
        ...
        for step, (samples, one_labels) in zip(progress_bar, train_ds):  # ✅ Fix infinite looping
            ...
            # 🚨 If NaN appears, restore from last checkpoint
            if np.isnan(loss_value):  
                logger.error("❌ NaN detected! Restoring last good checkpoint.")

                # 🔍 Find the last saved best model
                best_models = sorted(glob.glob("checkpoints/best_student_*.h5"))
                if best_models:
                    latest_best_model = best_models[-1]  # Get most recent best model
                    student_vit.load_weights(latest_best_model)  # Restore checkpoint
                    logger.info(f"✅ Restored from {latest_best_model}. Continuing training...")
                    nan_count += 1
                else:
                    logger.warning("⚠️ No previous best model found! Training cannot continue.")
                    break  # Stop if no backup

                if nan_count >= 3:  # 🚨 If NaN happens 3 times, stop training
                    logger.error("🚨 NaN appeared 3 times! Stopping training.")
                    break  

                continue  # ✅ Skip current batch but continue training

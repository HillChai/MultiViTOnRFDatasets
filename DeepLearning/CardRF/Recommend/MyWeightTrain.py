...
# 计算类别权重
class_weights = {i: np.sqrt(total_train_samples / count) for i, count in lable_cnt.items()}
...
def gen_2D_dataset(folder_path, batch_size, is_test=False):
    ...
    def data_generator():
        ...
        for file_path in all_files:
            try:
                ...
                num_selected = int(len(X_data) * sampling_rate)
                if num_selected < len(X_data):
                    selected_indices = np.linspace(0, len(X_data) - 1, num=num_selected, dtype=int)
                    X_data = X_data[selected_indices]
                    if not is_test:
                        w_data = w_data[selected_indices]
                ...
                if is_test:
                    ...
                    yield from zip(X_data, one_hot_labels, w_data)
    ...
    if is_test:
        dataset = tf.data.Dataset.from_generator(
            data_generator,
            output_signature=(
                tf.TensorSpec(shape=(sample_length,), dtype=tf.float32),
                tf.TensorSpec(shape=(len(labels_index),), dtype=tf.float32),
            )
        ).map(lambda x, y: (tf.reshape(x, [1, sample_length, 1]), y))
    else:
        dataset = tf.data.Dataset.from_generator(
            data_generator,
            output_signature=(
        ...
        ).map(lambda x, y, w: (tf.reshape(x, [1, sample_length, 1]), y, w))

    if is_test:
        return dataset.batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)

    train_dataset = dataset.skip(val_count).shuffle(buffer_size=batch_size * 10).repeat().batch(batch_size, drop_remainder=True).prefetch(tf.data.experimental.AUTOTUNE)
    val_dataset = dataset.take(val_count).map(lambda x, y, _: (x, y)).batch(batch_size).prefetch(tf.data.experimental.AUTOTUNE)
...
def train(is_old_training: bool,
    ...
    lr_schedule = tf.keras.optimizers.schedules.CosineDecayRestarts(
        initial_learning_rate=1e-9,  # 初始学习率
...
if __name__ == '__main__':
    train(is_old_training=True,
          train_path="/CardRFDataset/WeightCardRF/LOS/Train",
          test_path="/CardRFDataset/CardRF/LOS/Test",
          checkpoint_dir="/SaveFolders/Recommend/checkpoints",

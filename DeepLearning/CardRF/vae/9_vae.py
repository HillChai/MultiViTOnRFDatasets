...
# 2. 批量加载整个数据集
folder_path = "/CardRFDataset/CardRF/LOS/Train/UAV/DJI_MAVICPRO/FLYING/split/"  # 你的数据集文件夹路径
expand_num = 85400 - 42700  # 需要生成的数据量
current = 9
output_folder = f"/CardRFDataset/CardRF/LOS/Train/UAV/DJI_MAVICPRO/FLYING/split/{current}_expanded"  # 生成的数据存放目录
...
# 3. 归一化数据
X_all = (X_all - X_all.min()) / (X_all.max() - X_all.min()+ 1e-8)  # 归一化到 [0,1]
...
optimizer_ldm = optim.Adam(ldm.parameters(), lr=5e-4)
